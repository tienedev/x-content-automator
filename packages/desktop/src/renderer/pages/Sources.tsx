...
import { Plus, RefreshCw, Eye, Trash2, ExternalLink, Copy, Search } from 'lucide-react';
...
const Sources: React.FC = () => {
  ...
  const handleAddSource = async (): Promise<void> => {
    ...
    // setShowAddForm(false); // TODO: Implement add form functionality
  ...
  //       setSources(sources.map(s => (s.id === sourceId ? { ...s, ...updatedSource } : s)));
  //     }
  //   } catch (error) {
  ...
  const handleCopyContent = async (item: { title: string; description: string; link: string }) => {
  ...
  const addPredefinedSource = async (feedData: {
    name: string;
    url: string;
    type: SourceType;
    category: PostCategory;
        ...
        <div className='flex items-center justify-between mb-8'>
          <div>
            ...
            <p className='text-gray-600 dark:text-muted-foreground mt-1'>
              Gérez vos flux RSS et sources de contenu
            </p>
          </div>
              ...
              <Card className="bg-white dark:bg-card">
                <CardContent className='flex flex-col items-center justify-center py-12'>
                  ...
                  <p className='text-gray-600 dark:text-muted-foreground text-center max-w-md'>
                    Commencez par ajouter quelques sources RSS pour collecter automatiquement du
                    contenu
                  </p>
                  <Button
                    onClick={() => {
                    ...
                    >
                      <CardHeader className='pb-3'>
                        <div className='flex items-center justify-between'>
                          <div className='flex items-center gap-3'>
                            ...
                            <div>
                              <CardTitle className='text-base truncate max-w-[200px] text-gray-900 dark:text-foreground'>
                                {source.name}
                              </CardTitle>
                      ...
                      <CardContent className='pt-0'>
                        <div className='flex items-center justify-between text-xs text-gray-500 dark:text-muted-foreground mb-3'>
                          <span>{formatDate(source.lastUpdate)}</span>
                          <span>
                            {feed && !feed.error
                    ...
                    {getAllArticles().length === 0 ? (
                      <div className='text-center py-8 text-gray-500'>
                        <p>
                          ...
                          {selectedFilter !== 'all' ? ' pour cette source' : ''}
                        </p>
                      </div>
                    ) : (
                      <>
                        {selectedFilter !== 'all' && (
                          <div className='mb-4'>
                            <Badge variant='secondary' className='text-xs'>
                              {sources.find(s => s.id === selectedFilter)?.name} -{' '}
                              {getAllArticles().length} articles
                            </Badge>
                          </div>
                        )}
                          ...
                          return (
                            ...
                            >
                              <div className='flex-1'>
                                ...
                                <p className='text-xs text-gray-500 mt-1 truncate'>{feed.url}</p>
                              </div>
                              <div className='flex items-center gap-2'>
                                <Button
                                  variant='ghost'
