...
const SourceCategoryManager: React.FC<SourceCategoryManagerProps> = ({ onSourcesAdded }) => {
  ...
  const handleAddSelectedFeeds = async (categoryId: string) => {
    ...
    try {
      await window.electronAPI.storage.addSourcesFromCategory(categoryId, Array.from(selectedFeeds))
      onSourcesAdded()
      setSelectedCategory(null)
      setSelectedFeeds(new Set())
  ...
  const selectAllFeeds = (category: FeedCategory) => {
    const availableFeeds = category.feeds.filter(feed => 
      !existingSources.some(source => source.url === feed.url)
    )
                        ...
                        {availableFeeds.map((feed, index) => (
                          <label
                            ...
                            style={{
                              ...
                              padding: '10px',
                              borderRadius: '6px',
                              backgroundColor: selectedFeeds.has(feed.url) ? '#eff6ff' : 'transparent',
                              border: selectedFeeds.has(feed.url) ? '1px solid #3b82f6' : '1px solid transparent',
            ...
            return (
              ...
              >
                <div style={{
                  ...
                  <h4 style={{ fontSize: '16px', fontWeight: '600', margin: 0 }}>{category.name}</h4>
                </div>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    backgroundColor: existingCount === category.feeds.length ? '#10b981' : '#f59e0b',
