...
export function registerStorageHandlers(): void {
  ...
  ipcMain.handle(
    'storage:add-source',
    async (event, source: Omit<Source, 'id'>): Promise<Source> => {
  ...
  ipcMain.handle(
    ...
    async (event, id: string, updates: Partial<Source>): Promise<void> => {
  ...
  ipcMain.handle(
    'storage:add-sources-from-category',
    async (event, categoryId: string, selectedUrls?: string[]): Promise<void> => {
  ...
  ipcMain.handle(
    ...
    async (event, settings: Partial<AppSettings>): Promise<void> => {
