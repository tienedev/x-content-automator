...
const electronAPI: ElectronAPI = {
  ...
  storage: {
    ...
    addSourcesFromCategory: (categoryId, selectedUrls?) => ipcRenderer.invoke('storage:add-sources-from-category', categoryId, selectedUrls),
  ...
  // Event listeners
  onMenuAction: (callback: (action: string) => void) => ipcRenderer.on('menu-action', (event, action) => callback(action)),
