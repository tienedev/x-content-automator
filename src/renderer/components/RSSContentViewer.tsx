  ...
  if (activeSources.length === 0) {
    return (
      <div className="text-center p-10 text-gray-500 dark:text-muted-foreground">
        ...
        <p className="text-sm text-gray-500 dark:text-muted-foreground">Activez des sources RSS pour voir leur contenu ici</p>
      </div>
    )
  }
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        {isLoading ? (
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <p className="mt-4 text-gray-600 dark:text-muted-foreground">Chargement des flux RSS...</p>
          </div>
        ) : selectedFeed ? (
          <div>
            {/* En-tête du flux */}
            ...
            {selectedFeed.error ? (
              ...
              <div className="text-center p-5 text-gray-500 dark:text-muted-foreground">
                <p className="text-gray-500 dark:text-muted-foreground">Aucun article dans ce flux</p>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {selectedFeed.items.map((item, index) => (
                  <div key={index} className="p-4 border border-gray-200 dark:border-border rounded-lg bg-white dark:bg-card transition-shadow">
          ...
          <div className="text-center p-10 text-gray-500 dark:text-muted-foreground">
            <p className="text-gray-500 dark:text-muted-foreground">Sélectionnez une source pour voir son contenu</p>
          </div>
        )}
      </div>
    </div>
