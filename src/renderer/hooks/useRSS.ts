...
export const useRSS = () => {
  ...
  const fetchFeeds = useCallback(async (urls: string[]): Promise<RSSFeedResult[]> => {
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Erreur lors de la récupération des flux RSS'
  ...
  const fetchSourcesContent = useCallback(async (sources: Source[]): Promise<Map<string, RSSFeedResult>> => {
    const activeRSSSources = sources.filter(
      source => source.active && source.type === 'rss'
    )
  ...
  const testRSSUrl = useCallback(async (url: string): Promise<{ valid: boolean; error?: string }> => {
    try {
